...
export default function RouteInspector() {
  ...
  return (
    <div className="flex gap-5 h-[calc(100vh-140px)] min-h-[600px] font-sans text-slate-800">
      {/* ═══════════════════════════════════════════
          LEFT PANEL — Route List
      ═══════════════════════════════════════════ */}
      <div className="w-[340px] shrink-0 flex flex-col bg-white/70 backdrop-blur-md rounded-2xl border border-white/80 shadow-xs overflow-hidden">
        {/* Header */}
        <div className="p-4 pb-3 border-b border-slate-100">
          <h2 className="text-lg font-black text-slate-900">Routes</h2>
          ...
          <div className="relative mt-3">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 bg-slate-50 border border-slate-200/80 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs shadow-xs placeholder:text-slate-400 font-medium"
            />
          </div>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    {/* Badge */}
                    <div className={`w-10 h-10 rounded-xl ${route.badgeBg} text-white font-extrabold text-xs flex items-center justify-center shrink-0 shadow-xs`}>
                      {route.id}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        <span className={`w-1.5 h-1.5 rounded-full ${route.dotColor}`}></span>
                        <span className={`text-[10px] font-bold ${route.textColor}`}>{route.status}</span>
                        <span className="text-slate-300">·</span>
                        <span className={`text-[10px] font-extrabold ${route.healthColor}`}>{route.healthScore}%</span>
                      </div>
                    </div>
                  </div>
                </button>
        ...
        {!selectedRealId || detailLoading && !route ? (
          ...
          <>
            ...
            <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between shrink-0">
              ...
              {selectedRouteMeta && (
                <div className={`px-3 py-1 rounded-lg text-xs font-extrabold text-white ${selectedRouteMeta.badgeBg}`}>
            ...
            <div className="flex-1 grid grid-cols-12 gap-0 min-h-0">
              ...
              <div className="col-span-3 border-r border-slate-100 p-4 flex flex-col overflow-hidden">
                <h3 className="font-bold text-slate-900 text-xs mb-3">Route Timeline</h3>

                <div className="flex-1 overflow-y-auto pr-1 relative no-scrollbar">
                  <div className="absolute left-[9px] top-3 bottom-3 w-0.5 bg-slate-200 -z-10"></div>

                  {busStops.slice(0, 15).map((stop: any, index: number) => {
                    const isBuncedStop = isBunching && index === 2;
                    return (
                      <div key={stop.id} className="mb-5 relative z-10 flex gap-3 items-start">
                        <div className={`w-4.5 h-4.5 rounded-full border-[3px] border-white flex-shrink-0 shadow-xs ${isBuncedStop ? 'bg-red-500 ring-2 ring-red-100' : 'bg-slate-300'}`}></div>
                        <div className="-mt-0.5 flex-1 min-w-0">
                          <div className="font-bold text-slate-800 text-[11px] truncate">{stop.name}</div>
                          {isBuncedStop && (
                            <div className="mt-1.5 space-y-1 bg-red-50/60 p-2 rounded-lg border border-red-100">
                              <div className="flex flex-wrap gap-1">
                                <span className="bg-emerald-600 text-white text-[9px] font-extrabold px-1.5 py-0.5 rounded">{route.routeName} +7m</span>
                                <span className="bg-amber-500 text-white text-[9px] font-extrabold px-1.5 py-0.5 rounded">{route.routeName} +12m</span>
                              </div>
                              <div className="text-[10px] font-bold text-red-600 flex items-center gap-1">
                                <AlertTriangle size={11} /> Bunching detected
                              </div>
                            </div>
                          )}
                        </div>
                ...
                <div className="pt-3 border-t border-slate-100 mt-2 shrink-0">
                  <h4 className="text-[9px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider">Bus Spacing</h4>
                  <div className="relative h-1.5 bg-slate-100 rounded-full w-full">
                    <div className="absolute top-1/2 -translate-y-1/2 left-[10%] w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-white shadow-xs"></div>
                    <div className="absolute top-1/2 -translate-y-1/2 left-[40%] w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-white shadow-xs"></div>
                    <div className="absolute top-1/2 -translate-y-1/2 left-[43%] w-2.5 h-2.5 rounded-full bg-amber-500 border-2 border-white shadow-xs"></div>
                    <div className="absolute top-1/2 -translate-y-1/2 left-[90%] w-2.5 h-2.5 rounded-full bg-slate-300 border-2 border-white shadow-xs"></div>
                  </div>
                  <div className="text-center mt-2 text-[10px] font-bold text-red-500">Gap: 1.4 min (target: 8 min)</div>
                </div>
              </div>
              ...
              <div className="col-span-6 relative overflow-hidden">
                ...
                >
                  ...
                  {trips.filter((t: any) => t.liveTelemetry).map((trip: any) => (
                    <Marker key={trip.id} longitude={trip.liveTelemetry.lng} latitude={trip.liveTelemetry.lat}>
                      <div className="w-6 h-6 bg-blue-600 rounded-md shadow-md flex items-center justify-center text-white border-2 border-white">
                  ...
                  {isBunching && trips[0]?.liveTelemetry && (
                    <Marker longitude={trips[0].liveTelemetry.lng} latitude={trips[0].liveTelemetry.lat}>
                      <div className="w-24 h-24 bg-red-500/20 rounded-full flex items-center justify-center animate-pulse border border-red-500/30">
                        <div className="w-12 h-12 bg-red-500/30 rounded-full"></div>
                      </div>
                    </Marker>
                  )}
                </Map>
                ...
                {isBunching && (
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl px-4 py-2.5 shadow-lg flex items-center gap-3 min-w-[320px] z-20">
                    <div className="w-7 h-7 rounded-full bg-red-100 text-red-600 flex items-center justify-center shrink-0">
                      <AlertTriangle size={14} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-[11px] text-slate-900 truncate">B-{route.routeName}A & B-{route.routeName}B — 1.4 min apart</h4>
                      <p className="text-[10px] text-slate-500 mt-0.5">Target headway: 8 min · Traffic Congestion</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="col-span-3 border-l border-slate-100 p-4 flex flex-col overflow-y-auto no-scrollbar">
                <div className="mb-4">
                  <h2 className="text-sm font-bold text-slate-900">AI Copilot</h2>
                  <p className="text-[10px] text-slate-500 mt-0.5">Automated headway & congestion monitor</p>
                </div>

                {/* Likely Causes */}
                <div className="mb-4">
                  <h4 className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2">Likely Causes</h4>
                ...
                {/* Recommendation */}
                <div className="mb-4">
                  <h4 className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2">Recommendation</h4>
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-200/80">
                    <p className="text-[11px] font-bold text-slate-900 mb-2 leading-snug">Hold bus {route.routeName} at stop 14 for 2 mins</p>
                    <div className="flex gap-2">
                      <button
                        onClick={handleApprove}
                        className={`flex-1 font-bold py-1.5 rounded-lg text-[11px] flex items-center justify-center gap-1 transition-all cursor-pointer shadow-xs ${
                          isApproved === true ? 'bg-emerald-600 text-white ring-2 ring-emerald-200' : 'bg-emerald-500 hover:bg-emerald-600 text-white'
                        }`}
                      >
                        <Check size={12} /> {isApproved === true ? 'Approved' : 'Approve'}
                      </button>
                      <button
                      ...
                      >
                        <X size={12} /> {isApproved === false ? 'Rejected' : 'Reject'}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-auto">
                  <div className="bg-blue-50/70 rounded-xl p-3 border border-blue-100">
                    <h4 className="text-[9px] font-black text-blue-600 uppercase tracking-wider mb-1">Root Cause Summary</h4>
